...
void multiply(int process, int numberOfProcess, int timeLimit, int isDistinct, int *m_counter){
    timerStart();
    (*m_counter) = 0;
...
int main(int argc, char** argv){
    if (argc != 7){
        printf("Wrong number of arguments. Expected: [path] [number of process] [time limit] [save mode: commonFile / distinctFiles] [hard time] [hard memory]");
        exit(EXIT_FAILURE);
    }
    char *lista = argv[1];
    ...
    if (hardTime <= 0 || hardMemory <= 0){
        printf("Hard time and hard memory must be positive\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < numberOfProcess; i ++){
        ...
        if (childPid == 0){
            ...
            time_limit -> rlim_cur = hardTime;
            time_limit -> rlim_max = hardTime;
            ...
            multiply(i, numberOfProcess, timeLimit, isDistinct, &m_counter);
            free(time_limit);
            free(memory_limit);
            exit(m_counter);
        }
    ...
    for (int i = 0; i < numberOfProcess; i++){
        ...
        if (getrusage(childPids[i], usage) == RUSAGE_CHILDREN){
